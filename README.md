...
Devices must support at least one locale. Every bundle a device serves MUST be complete: it contains an entry for every `id` any
`LocalizedText` produced by the device can reference. This keeps rendering a single-subscription operation — the display never has
to pull an additional locale just to fill a gap.

If a display subscribes with a `locale` key the device does not serve, the device responds with a `LocaleBundle` under one of its
own supported locales rather than nothing: the `locale` field in the response tells the display what was actually served, and
`availableLocales` lets the display pick differently next time. This keeps bundle discovery to a single round-trip.

##### Rendering `LocalizedText`

A display renders a `LocalizedText` value `lt` by:

1. Looking up `lt.id` in the bundle cached for `(lt.source, userLocale)`. Because bundles are required to be complete, a hit is
   expected for any id the source device actually produces; a miss indicates a firmware/display mismatch and the display renders
   the raw `lt.id` as a last resort.
2. Substituting `lt.parameters` into the format string under the subset rules below.
