...
DeviceData {
   name:              LocalizedText,         // Name of the device
   description:       LocalizedMarkdownText, // Description of the device and its operation
...
Provide localized message bundles used to render `LocalizedText` values produced by this device.

```
Modality(
   id          = "scan.i18n",
   priority    = Management,
   keyType     = String,                         // BCP 47 locale tag, e.g. "en", "en-US", "de"
   outputType  = LocaleBundle,
   inputType   = Unit
)

LocaleBundle {
   locale:            String,                    // Locale actually served (may differ from the subscribed key if the device fell back)
   bundleVersion:     String,                    // Opaque version tag; caches are keyed by (source peer, locale, bundleVersion)
   availableLocales:  Array(String),             // All locale tags this device can serve
   fallback:          Array(String),             // Ordered fallback locale chain for messages missing from this bundle
   messages:          Stream(LocaleMessage)
}

LocaleMessage {
   id:      String,                              // Matches LocalizedText.id
   format:  String                               // Format string; see the subset defined below
}
```

This modality is the source of truth for rendering every `LocalizedText` value whose `source` is this peer. A device never translates
its own text: it emits a `LocalizedText` carrying a `source`, an `id`, and typed `parameters`, and the display device (administrative
app, wall panel, phone) fetches the appropriate bundle from the source peer and performs substitution locally. Bundles travel only
with firmware — they are generated by the vendor as part of the build — so no internet connection, registry, or translation service
is ever required at runtime.

Devices must support at least one locale. If a display subscribes with a `locale` key the device does not serve, the device responds
with a `LocaleBundle` under its own fallback locale rather than nothing: the `locale` field in the response tells the display what
was actually served, and `availableLocales` lets the display pick differently next time. This keeps bundle discovery to a single
round-trip.

Because bundles change only with firmware, displays should cache them keyed by the triple `(source, locale, bundleVersion)`. If a
later subscription returns the same `bundleVersion`, the cached bundle is still authoritative and no re-download is needed.

##### Rendering `LocalizedText`

A display renders a `LocalizedText` value `lt` by:

1. Looking up `lt.id` in the bundle for `(lt.source, userLocale, *)`.
2. If the id is absent, walking `fallback` (locales from the same source) until a match is found. If no match exists, rendering
   the raw `lt.id` as a last resort.
3. Substituting `lt.parameters` into the format string under the subset rules below.

A parameter of variant `Text` is itself a `LocalizedText` and is rendered recursively; its own `source` may differ from the outer
message, so a controller can refer to a sensor whose display name lives in the sensor's own bundle and the display composes the
final string across peers. Rendering depth must be bounded (implementations may cap at 8 nesting levels); cycles cause the
offending parameter to render as its raw `id`.

Non-`Text` parameter variants (`Integer`, `Number`, `Timestamp`, `Duration`, `Raw`) are formatted by the display according to the
...
* Plural: `{count, plural, =0 {no items} =1 {one item} other {# items}}`. The `#` token renders the argument in place. Explicit
  ...
  * Keyword arms `one` and `other` MUST be understood for every locale the device serves. A runtime without CLDR plural rule
    data treats `one` as "argument equals 1" and everything else as `other`, which is correct for English and other Germanic
    locales.
  * Keyword arms `zero`, `two`, `few`, `many` are OPTIONAL; a runtime that does not recognise them falls through to `other`.
...
// A user-facing message produced by a device. The device does not translate
// its own text; it emits a reference that display devices resolve against
// the source peer's scan.i18n bundles. See the Internationalization modality
// for bundle format and rendering rules.
LocalizedText {
   source:      PeerAddress,                     // Peer whose scan.i18n owns the bundle
   id:          String,                          // Message id within that bundle
   parameters:  DynamicArray(MessageParameter)
}

MessageParameter {
   name:   String,                               // Parameter name as referenced from the format string
   value:  ParameterValue
}

ParameterValue = Integer    { value: SignedInteger(8) }
              | Number     { value: FloatingPoint(8) }
              | Timestamp  { value: Timestamp }
              | Duration   { value: Duration }
              | Raw        { value: String }          // Opaque human string (user-entered names, identifiers, pre-formatted values)
              | Text       { value: LocalizedText }   // Recursive localized reference; bounded nesting depth

// Same structure as LocalizedText; the marker exists so administrative
// interfaces know the resolved output should be rendered as CommonMark
// rather than plain text.
LocalizedMarkdownText = LocalizedText
