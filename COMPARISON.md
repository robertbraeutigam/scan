...
|----------|-----------|-------------|---------------|--------------|-----------|
| **SCAN** | TCP/IP + UDP multicast | **Unlimited** (chunked, 1200 B per frame) | Built-in (chunk-based, per-chunk encrypted) | Yes (multiple logical connections per TCP) | **Yes** (native) |
...
3. **Constrained device suitability is unproven** -- TCP + Noise crypto + all mandatory modalities are still heavy for the smallest microcontrollers, even though the 1200-byte per-frame cap means per-connection buffers no longer exclude Class 2 MCUs (ESP8266, nRF52832, Cortex-M4 with ≥64 KB SRAM). CoAP on an 8-bit MCU with 32 KB flash is proven; SCAN on such hardware is not.
